...
/* ─────────── Helper: Format & Animate Lyrics ─────────── */
// ฟังก์ชันนี้จะจัดการลบขีด/เส้น/สัญลักษณ์ทุกรูปแบบด้านหน้า และเปลี่ยนบรรทัดว่างเป็นอนิเมชันจุด
function renderLyricText(txt, isFullTime = false) {
  if (typeof txt !== "string") return "\u00A0";

  // ลบขีด (hyphen, en-dash, em-dash, fullwidth dash), สัญลักษณ์ หรือช่องว่างหน้าข้อความทั้งหมด
  const cleanTxt = txt.replace(/^[\s\-\–\—\―\•\>\│\|\:\.]+/g, "").trim();

  // เช็คว่าเป็นท่อนบรรเลงดนตรีหรือบรรทัดว่างหรือไม่
  const isInstr = cleanTxt === "" || cleanTxt === "♪" || cleanTxt.toLowerCase() === "instrumental";

  if (isInstr) {
    return (
      <span className="instrumental-dots" style={{ display: "inline-flex", gap: isFullTime ? "10px" : "6px", alignItems: "center", height: "1em", verticalAlign: "middle" }}>
        {[0, 1, 2].map((dot) => (
          <motion.span
            key={dot}
            animate={{ y: [0, isFullTime ? -8 : -5, 0], opacity: [0.3, 1, 0.3] }}
            transition={{ duration: 1.2, repeat: Infinity, delay: dot * 0.2, ease: "easeInOut" }}
            style={{
              width: isFullTime ? 8 : 6,
              height: isFullTime ? 8 : 6,
              borderRadius: "50%",
...
function NoLyricsArt({ reason, animSpeed = 1 }) {
  ...
  const configs = {
    not_found: { title: "ไม่พบเนื้อเพลง", sub: "ไม่มีเนื้อเพลงสำหรับบทเพลงนี้" },
    idle: { title: "ยังไม่ได้เล่นเพลง", sub: "เปิดเพลงใน Spotify เพื่อเริ่มต้น" },
    error: { title: "เกิดข้อผิดพลาด", sub: "ไม่สามารถโหลดเนื้อเพลงได้" },
    hidden: { title: "เนื้อเพลงถูกซ่อน", sub: "เปิดจากแถบตั้งค่าด้านบน" },
  };
  ...
  return (
    ...
    >
      ...
      <motion.div className="no-lyrics-text"
        initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: dur(0.4) }}
      >
...
export default function LyricsView({
  ...
  const measureShift = useCallback((retryCount = 0) => {
    ...
    if (!container || !lineEl || container.clientHeight === 0) {
    ...
    if (isUserScrolling.current) {
      const diff = target - shiftRef.current;
      if (diff !== 0) {
        setManualOffset((prev) => prev - diff);
      }
    }
  ...
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const observer = new ResizeObserver(() => measureShift());
  ...
  const onWheel = useCallback((e) => handleUserScroll(e.deltaY), [handleUserScroll]);
  const onTouchStart = useCallback((e) => { lastTouchY.current = e.touches[0].clientY; }, []);
  ...
  if (!hasContent) {
    return (
      <AnimatePresence mode="wait">
        {status === "loading" ? (
          <motion.div key="loading" className="lyrics-empty-state" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="no-lyrics-rings">
              {[0, 1, 2].map((i) => (
            ...
            <motion.div className="lyrics-spinner" animate={{ rotate: 360 }} transition={{ duration: dur(1), repeat: Infinity, ease: "linear" }} />
            <motion.span className="empty-text" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              กำลังค้นหาเนื้อเพลง...
            </motion.span>
          </motion.div>
        ) : (
        ...
        {viewMode === "fulltime" ? (
          ...
          >
            {synced ? (
              ...
              >
                {/* Intro: ใช้ inset: 0 เพื่อแก้บัคขอบทะลุ */}
                <AnimatePresence mode="wait">
                  {currentLineIndex === -1 && !songEnded && (
                    <motion.div
                      ...
                      exit={{
                        ...
                        y: -25,
                        filter: "blur(16px)",
                        transition: { duration: dur(0.45), ease: [0.32, 0, 0.67, 0] }
                      }}
                      transition={{ duration: dur(0.55), ease: [0.16, 1, 0.3, 1] }}
                      style={{
                        position: "absolute",
                        inset: 0, // <-- แก้ตรงนี้เป็น inset: 0
                        display: "flex",
                    ...
                    >
                      <div className="lyrics-intro-dots" style={{ display: "flex", gap: 10, alignItems: "center" }}>
                        {[0, 1, 2].map((i) => (
                          <motion.span
                            key={i}
                            className="lyrics-intro-dot"
                            animate={{ y: [0, -6, 0], opacity: [0.3, 1, 0.3], scale: [0.9, 1.2, 0.9] }}
                            transition={{ duration: dur(1.2), repeat: Infinity, delay: i * 0.18, ease: "easeInOut" }}
                            style={{ width: 9, height: 9, borderRadius: "50%", background: "currentColor", boxShadow: "0 0 10px rgba(255,255,255,0.4)" }}
                          />
                        ))}
                      </div>
                      <motion.span
                        className="lyrics-intro-text"
                        animate={{ opacity: [0.6, 1, 0.6] }}
                        transition={{ duration: dur(2), repeat: Infinity, ease: "easeInOut" }}
                        style={{ fontSize: 15, fontWeight: 500, color: "rgba(255,255,255,0.7)", letterSpacing: "0.04em" }}
                      >
                        เพลงกำลังจะเริ่ม
                      </motion.span>
                    </motion.div>
                  ...
                  {songEnded && (
                    <motion.div
                      ...
                      style={{
                        ...
                        alignItems: "center", justifyContent: "center", gap: 10, pointerEvents: "none", zIndex: 10
                      }}
                    >
                      <motion.div
                    ...
                    {synced.map((line, i) => {
                      ...
                      return (
                        <motion.p
                          ...
                          style={{ transformOrigin: "left center", borderLeft: "none", border: "none" }}
                        >
                          {renderLyricText(line.text)}
                        </motion.p>
                      );
                    })}
                    <div className="lyrics-spacer" />
              ...
              <div className="lyrics-scroll lyrics-plain">
                <AnimatePresence mode="wait">
                  ...
                  >
                    {plain.split("\n").map((line, i) => (
                      <p key={i} className="lyric-line plain" style={{ borderLeft: "none", border: "none" }}>
                        {renderLyricText(line)}
                      </p>
                    ))}
                  </motion.div>
                </AnimatePresence>
...
function FullTimeDisplay({ synced, currentLineIndex, trackId, animSpeed = 1, songEnded = false }) {
  ...
  return (
    <div className="fulltime-display" style={{ position: "relative", width: "100%", height: "100%" }}>
      <AnimatePresence mode="wait">
        {(isIntro || songEnded) && (
          <motion.div
            key={isIntro ? "fulltime-intro" : "fulltime-outro"}
            initial={{ opacity: 0, scale: 0.88, y: 20, filter: "blur(14px)" }}
            animate={{ opacity: 1, scale: 1, y: 0, filter: "blur(0px)" }}
            exit={{
              opacity: 0,
              scale: 1.12,
          ...
          >
            {isIntro ? (
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                {[0, 1, 2].map((i) => (
                  <motion.span
                    key={i}
                    animate={{ y: [0, -8, 0], opacity: [0.3, 1, 0.3], scale: [0.9, 1.25, 0.9] }}
                    transition={{ duration: dur(1.2), repeat: Infinity, delay: i * 0.18, ease: "easeInOut" }}
                    style={{ width: 10, height: 10, borderRadius: "50%", background: "currentColor", boxShadow: "0 0 12px rgba(255,255,255,0.4)" }}
                  />
                ))}
              </div>
            ) : (
            ...
            <motion.span
              animate={{ opacity: [0.6, 1, 0.6] }}
              transition={{ duration: dur(2), repeat: Infinity, ease: "easeInOut" }}
              style={{ fontSize: 16, fontWeight: 500, color: "rgba(255,255,255,0.7)", letterSpacing: "0.05em" }}
            >
      ...
      >
        ...
        <div className="fulltime-bg-words">
          {synced?.slice(Math.max(0, currentLineIndex - 2), currentLineIndex + 5).map((line, i) => (
            <motion.span key={`bg-${trackId}-${currentLineIndex}-${i}`}
              ...
              style={{ "--wi": i }}
            >{renderLyricText(line.text)}</motion.span>
          ))}
        </div>
        ...
        <AnimatePresence mode="popLayout">
          {prev && (
            ...
            >
              {renderLyricText(prev.text, true)}
            </motion.p>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence mode="popLayout">
          ...
          >
            {renderLyricText(curr?.text, true)}
          </motion.p>
        </AnimatePresence>
        ...
        <AnimatePresence mode="popLayout">
          {next && (
            ...
            >
              {renderLyricText(next.text, true)}
            </motion.p>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence mode="popLayout">
          {next2 && (
            ...
            >
              {renderLyricText(next2.text, true)}
            </motion.p>
          )}
        </AnimatePresence>
