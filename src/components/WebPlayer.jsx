 ...
 * ให้ browser เป็น Spotify device แทน (ต้องการ Premium)
 */
export default function WebPlayer({ animSpeed = 1 }) {
  ...
  const initPlayer = () => {
    ...
    window.onSpotifyWebPlaybackSDKReady = () => {
      ...
      player.addListener("ready", ({ device_id }) => {
        ...
        setStatus("ready"); // Wait for user to click to start playback
      });
  ...
  const startPlayback = () => {
    if (playerRef.current) {
      // Unlock audio context explicitly per Spotify SDK requirements
